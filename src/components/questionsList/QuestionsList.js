    ...
    <>
      ...
      <div className="container">
        <div className="question">
          <h1>{DUMMY_QA[currentQuestion].prompt}</h1>
          <h2>{DUMMY_QA[currentQuestion].question}</h2>
        </div>
        <div className="answer">
          <div>Select only one</div>
          <div>
            <button onClick={() => setChosen("A")}>
              {DUMMY_QA[currentQuestion].optionA}
            </button>
          </div>
          <div>
            <button onClick={() => setChosen("B")}>
              {DUMMY_QA[currentQuestion].optionB}
            </button>
          </div>
          <div>
            <button onClick={() => setChosen("C")}>
              {DUMMY_QA[currentQuestion].optionC}
            </button>
          </div>
          <div>
            <button onClick={() => setChosen("D")}>
              {DUMMY_QA[currentQuestion].optionD}
            </button>
          </div>
        </div>
      </div>
